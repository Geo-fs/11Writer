...
def test_data_ai_multi_feed_route_parses_registry_fixtures_and_preserves_caveats() -> None:
    ...
    assert payload["metadata"]["rawCount"] == 7
    assert payload["metadata"]["dedupedCount"] == 6
...
def test_data_ai_multi_feed_source_filter_and_limit() -> None:
    ...
    payload = client.get(
        ...
        params={"source": "cloudflare-status,gdacs-alerts", "limit": 1},
    ).json()

    assert payload["metadata"]["selectedSourceIds"] == ["cloudflare-status", "gdacs-alerts"]
